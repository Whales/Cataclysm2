...
// Detect all valid targets.
  ...
  for (p.x = user->pos.x - range; p.x <= user->pos.x + range; p.x++) {
    for (p.y = user->pos.y - range; p.y <= user->pos.y + range; p.y++) {
      ...
      if (ent) {
        ...
        } else {  // Not an enemy and not ourselfs; must be a friend!
          if (affect_friends) {
            use_ent = true;
          }
        }
  ...
  if (targets.empty()) {  // Couldn't find anything!
    return false;
  }
...
{
  switch (type) {
    case MON_ABILITY_NULL:      return "NULL";
    case MON_ABILITY_SUMMON:    return "summon";
    case MON_ABILITY_SIGNAL:    return "signal";
    case MON_ABILITY_TERRAIN:   return "terrain";
    case MON_ABILITY_TELEPORT:  return "teleport";
    case MON_ABILITY_FIELDS:    return "fields";
    case MON_ABILITY_MAX:       return "ERROR - MON_ABILITY_MAX";
    default:                    return "ERROR - Unnamed Monster_ability_type";
  }
